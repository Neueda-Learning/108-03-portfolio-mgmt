...
const normalizeUser = (user) => {
    ...
    const fullName = [user?.name, [user?.firstname, user?.lastname].filter(Boolean).join(' ').trim(), user?.email]
        .find(Boolean)
        ?.trim()
...
function Navbar({ pageTitle = 'Dashboard' }) {
    ...
    const normalizedUsers = useMemo(() => {
        const source = apiUsers.length > 0 ? apiUsers : users
        return (source || []).map(normalizeUser).filter((u) => u.id != null)
    }, [apiUsers, users])

    useEffect(() => {
        const selectedId = selectedUser?.id ?? selectedUser?.userId
        if (selectedId != null) {
            setActiveUserId(String(selectedId))
            return
        }

        if (!activeUserId && normalizedUsers[0]?.id != null) {
            setActiveUserId(String(normalizedUsers[0].id))
        }
    }, [selectedUser, normalizedUsers, activeUserId])

    const profileName = useMemo(() => {
        const selected = normalizedUsers.find((user) => String(user.id) === activeUserId)
        return selected?.name || normalizeUser(selectedUser || {})?.name || 'User'
    }, [activeUserId, selectedUser, normalizedUsers])
            ...
            <div className="flex flex-col gap-[7px]">
                ...
                <select
                    ...
                    onChange={(event) => {
                        ...
                        const selected = normalizedUsers.find((user) => String(user.id) === event.target.value)
